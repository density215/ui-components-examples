...
export class ProbesHexbinMap extends React.Component {
  ...
  componentDidMount() {
    const countries = loadCountryGeoInfo({
      detail: "10m",
      places: null,
      geoKey: "openipmapCountries-ne10m",
    }).then(
      (countries) => {
        console.log("loading world data...");
        console.log(countries);
        const ripeCountries = countries.filter((c) =>
          ["NL", "FR", "BE", "GB", "DE", "DK", "LU"].some(
            (cc) => cc === c.properties.countryCode
          )
        );
        console.log(ripeCountries);
        this.setState({ countries: ripeCountries });
      },
      (error) => {
        console.log("error");
        console.log(error);
    ...
    const probesData = loadProbesInfo({
      // s[1] -> IPv4
      // S[2] -> IPv6
      probesFilter: (s) => s[3] === "NL" && s[2] && s[8] !== 0,
    }).then(
      (probesData) => {
  ...
  render() {
    return (
      <GeoMap
        ...
        width={1480}
        height={942}
        rotate={[-1.6047711641393203, -50.11205495022325]}
        zoom={72}
        move={[-15.313779054381001, 6.877571165297356]}
        translate={[740, 471]}
